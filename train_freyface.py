...
def compute_elbo(x, mean, log_var, out_mean, out_logvar):
    """ ELBO(Evidence Lower Bound) is the objective of VAE.
        We train the model just to maximize the ELBO.  """
...
#        """
        # Save samples generated by the model as the training goes on
        if epoch == 1 or epoch == 2 or epoch == 4 or epoch == 8 or epoch == 16 or epoch == 32 or epoch == 64 or epoch == 128:
            counter += 1
            generated_imgs = model.decode(noise)[0].view(-1, 1, 28, 20)
